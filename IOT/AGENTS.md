...
### 12. Temporary test configuration

- If a configuration or environment file must be changed temporarily for testing, restore it to its previous value after the test is complete unless the user explicitly asks to keep the change.
- Document any temporary configuration changes in docs/IMPLEMENTATION_LOG.md, including the original value, test value, and whether it was restored.
- Do not leave local, staging, or production API URLs changed only because they were useful for a smoke test.
